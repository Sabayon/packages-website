...
class Mirrors(Database):
    ...
    def continent_mirrors(self, continent):
        ...
        self.execute_query("""
        SELECT
            http_t.field_mirror_http_uri_url as url
        FROM
        ...
        SELECT
            ftp_t.field_mirror_ftp_uri_url as url
        FROM
            field_data_field_mirror_ftp_uri as ftp_t,
            field_data_field_use_in_router as router_t,
            field_data_field_continent as continent_t
        WHERE
            continent_t.field_continent_value = %s AND
            ftp_t.entity_id = router_t.entity_id AND
            ftp_t.entity_id = continent_t.entity_id AND
            router_t.field_use_in_router_value = "1" AND
            router_t.deleted = "0"
        """, (continent, continent,))
        data = self.fetchall()
        mirrors = [x['url'] for x in data]
        return mirrors
